                        ...
                        <tr id="{{$banner->banner_top_id}}">
                            ...
                            <td class="edit-links">
                                @foreach($lang_list as $lang_key => $one_lang)
                                    <a href="{{urlForFunctionLanguage($lang, Str::slug($banner->name).'/edititem/'.$banner->banner_top_id.'/'.$one_lang->id)}}" {{ !empty(IfHasName($banner->banner_top_id, $one_lang->id, 'banner_top')) ? 'class=active' : ''}}>{{$one_lang->lang}}</a>
                                @endforeach
                            </td>
                            @if($groupSubRelations->active == 1)
                            ...
                            @if($groupSubRelations->del_to_rec == 1)
                                <td class="check-destroy-element">
                                    <input type="checkbox" class="remove-all-elements"
                                           ...
                                           url="{{urlForFunctionLanguage($lang, Str::slug($banner->name).'/destroyBannerToCart')}}">
                                </td>
                            @endif
                        </tr>
                    @endforeach
