                        ...
                        <tr id="{{$one_info_line->info_line_id}}">
                            ...
                            <td class="edit-links">
                                @foreach($lang_list as $lang_key => $one_lang)
                                    <a href="{{urlForFunctionLanguage($lang, Str::slug($one_info_line->name).'/editinfoline/'.$one_info_line->info_line_id.'/'.$one_lang->id)}}" {{ !empty(IfHasName($one_info_line->info_line_id, $one_lang->id, 'info_line')) ? 'class=active' : ''}}>{{$one_lang->lang}}</a>
                                @endforeach
                            </td>
                            @if($groupSubRelations->active == 1)
                                ...
                                @if(CheckIfInfoLineHasItems($one_info_line->info_line_id)->isEmpty())
                                    <td class="check-destroy-element">
                                        <input type="checkbox" class="remove-all-elements"
                                               ...
                                               url="{{urlForFunctionLanguage($lang, Str::slug($one_info_line->name).'/destroyInfoLineToCart')}}">
                                    </td>
                                @else
